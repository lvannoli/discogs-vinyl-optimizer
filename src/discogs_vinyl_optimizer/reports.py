                ...
                "offers": [
                    {
                        ...
                        "allocated_shipping": str(allocated_shipping[index]),
                        "album_total_estimate": str(offer.item_price + allocated_shipping[index]),
...
def write_options_html(options: Iterable[PurchaseOption], path: str | Path, warnings: list[str] | None = None) -> None:
    ...
    body = [
        ...
        ".total{font-size:1.15rem;font-weight:700}.warning{background:#fff4d6;border:1px solid #e0bd54;padding:10px;margin:8px 0}",
        "</style></head><body>",
    ...
    for option in options:
        ...
        for index, offer in enumerate(option.offers):
            ...
            allocated = allocated_shipping[index]
            body.append("<tr>")
